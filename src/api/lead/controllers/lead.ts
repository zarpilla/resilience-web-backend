...
export default factories.createCoreController(
  ...
  ({ strapi }) => ({
    async create(ctx) {
      ...
      const emailData = {
        to: emailConfigData.leadEmail1To.split(","),
        from: process.env.SMTP_DEFAULT_FROM,
        replyTo: process.env.SMTP_DEFAULT_REPLY_TO,
        subject: emailConfigData.leadEmail1Subject,
        text: emailConfigData.leadEmail1
          .replace("{name}", data.name)
          .replace("{email}", data.email)
          .replace("{sector}", data.sector)
          .replace("{subject}", data.subject)
          .replace("{message}", data.message),
        html: emailConfigData.leadEmail1
          .replace("{name}", data.name)
          .replace("{email}", data.email)
          .replace("{sector}", data.sector)
          .replace("{subject}", data.subject)
      ...
      const emailData2 = !emailConfigData.leadEmail2Subject
        ? null
        : {
            to: data.email,
            from: process.env.SMTP_DEFAULT_FROM,
            ...
            replyTo: process.env.SMTP_DEFAULT_REPLY_TO,
            subject: emailConfigData.leadEmail2Subject,
            text: emailConfigData.leadEmail2
              .replace("{name}", data.name)
              .replace("{email}", data.email)
              .replace("{sector}", data.sector)
              .replace("{subject}", data.subject)
            ...
            html: emailConfigData.leadEmail2
              .replace("{name}", data.name)
              .replace("{email}", data.email)
              .replace("{sector}", data.sector)
              .replace("{subject}", data.subject)
